...
def load_data(data_path, data_type="train"):
    ...
    """
    if data_type == "train" or data_type == "valid" or data_type == 'temp':
        datas = []
...
def data_generator(data_type="temp"):
    """
    读取数据，生成DataLoader。
    """
    ...
    if data_type == "train":
        ...
        temp_data_path = os.path.join(config["data_home"], config["exp_name"], config["temp_data"])
        temp_data = load_data(temp_data_path, "temp")
        for d in temp_data:
            print(d)
